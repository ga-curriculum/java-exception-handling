...
| Aspect             | `throws`                                      | `throw`                                          |
| :----------------- | :-------------------------------------------- | :----------------------------------------------- |
| **Purpose**        | Declares the exceptions a method might cause. | Generates an exception during program execution. |
| **Usage location** | As a part of the method declaration.          | Inside any statement block.                      |
...
In this example, we'll look at a program that divides two numbers for us and displays the result. Open up the `Division` starter code in the <code class="filepath">./src/main/java/org/example/Division.java</code> file.
